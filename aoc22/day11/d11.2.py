...
class Monkey:
  ...
  def throws_to(self, init_lvl: int) -> tuple[int, int]:
    worry_lvl = self.calc_worry_lvl(init_lvl)

    if worry_lvl % self.test_nr == 0:
      return self.first_friend, worry_lvl
    return self.second_friend, worry_lvl

  def receive_item(self, item: int, prod_dividers: int) -> None:
    self.items.put(item % prod_dividers)

  def perform_shenanigans(self, monkeys, prod_dividers: int):
    while self.items.empty() == False:
      item = self.items.get()
      throws_to, new_worry_lvl_item = self.throws_to(item)

      monkeys[throws_to].receive_item(new_worry_lvl_item, prod_dividers)
...
for i in range(1, len(input), 7):
  ...
  monkey_dividers.append(test_nr)
  monkeys.append(
    Monkey(
      items,
      operation_sign,
...
prod_dividers = math.prod(monkey_dividers)
for _ in range(10_000):
  for monkey in monkeys:
    monkey.perform_shenanigans(monkeys, prod_dividers)
...
print(f"Part 2: {math.prod(highest)}")
